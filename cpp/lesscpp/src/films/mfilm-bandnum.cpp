...
public:
	...
	MFilm(const Properties &props) : Film(props) {
		...
		m_band_num = props.getInteger("bandnumber", 1);
	...
	void develop(const Scene *scene, Float renderTime) {
		...
		if (m_fileFormat == EMathematica || m_fileFormat == EMATLAB) {
			...
			//读取特定波段，因为npy中是一个像素一个像素按行存的，每个像素可能有多个波段
			unsigned int nels = 1;
			for (unsigned int i = 0; i < N; i++) nels *= shape[i];
			int img_size = shape[0] * shape[1];
			Float* banddata = new Float[nels];
			for (int i = 0; i < img_size; i = i + 1)
			{
				for (int j = 0; j < m_band_num; j++)
				{
					banddata[i*m_band_num + j] = data[i*bitmap->getChannelCount() + j];
				}
			}
			cnpy::npy_save(filename.string(), banddata, shape_ptr, N, "w");
			delete[] banddata;
		}
		else
		{
