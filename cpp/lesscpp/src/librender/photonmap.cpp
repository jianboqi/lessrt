...
void PhotonMap::dumpText(const std::string &filename) {
	std::ofstream os(filename.c_str());
	for (size_t i = 0; i<m_kdtree.size(); ++i) {
		const Point &p = m_kdtree[i].getPosition();
		const Vector &dir = m_kdtree[i].getDirection();
		os << p.x << " " << p.y << " " << p.z << " " << dir.x << " " << dir.y << " " << dir.z << m_kdtree[i].getPower().toString()<< endl;
	}
	os.close();
}
