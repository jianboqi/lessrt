...
SceneHandler::SceneHandler(const ParameterMap& params,
	...
	m_tags["bioemitter"] = TagEntry(EBioemitter, MTS_CLASS(Bioemitter));
...
void SceneHandler::endElement(const XMLCh* const xmlName) {
	...
	case EString: {
		...
		if (hasValue == hasFilename) {
			XMLLog(EError, "<string>: please provide one of 'value' or 'filename'");
		}
		else if (hasFilename) {
			...
			fs::ifstream is(path);
			if (is.bad() || is.fail())
				SLog(EError, "<string>: could not open \"%s\"",
					path.string().c_str());

			std::string str((std::istreambuf_iterator<char>(is)), std::istreambuf_iterator<char>());

			SLog(EInfo, "\"%s\": loaded a string", path.filename().string().c_str());
