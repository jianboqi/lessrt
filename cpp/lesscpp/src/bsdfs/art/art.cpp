...
class ART : public BSDF {
public:
	ART(const Properties& props)
		:BSDF(props) {
		m_particle_size = props.getFloat("particle_size", 100);
		m_pollution_content = props.getFloat("pollution_content", 0)*1e-9;
		m_wavelengths = props.getSpectrum("wavelengths");
	}

	ART(Stream* stream, InstanceManager* manager)
		: BSDF(stream, manager) {
		m_particle_size = stream->readFloat();
		m_pollution_content = stream->readFloat();
	...
	void serialize(Stream* stream, InstanceManager* manager) const {
		...
		stream->writeFloat(m_particle_size);
		stream->writeFloat(m_pollution_content);
	...
	void configure() {
		...
		for (int i = 0; i < SPECTRUM_SAMPLES; i++) {
			...
			Float deltaY = RefractionIndex[right_index] - RefractionIndex[left_index];
			m_refrections[i] = deltaY * (wavelength - left_wavelength) + RefractionIndex[left_index];
		}
	}
	...
	Spectrum computeDirectinalRelectance(const BSDFSamplingRecord& bRec) const {
		...
		Float bb = 1.186;
		Float cc = 5.157;
		Float costerm = -cosThetai * cosThetar + sinThetai * sinThetar * cosdPhi;
		Float BigTheta = math::safe_acos(costerm);
		...
		for (int i = 0; i < SPECTRUM_SAMPLES; i++) {
			Float wavelength = m_wavelengths[i];
			Float alpha = 5.8 * math::safe_sqrt(4 * M_PI_DBL * m_particle_size * (m_refrections[i] + 0.2 * m_pollution_content) / wavelength * 1000);
			Float rso = r0 * math::fastexp(-alpha * Ks * Kv / r0);
			reflectance[i] = rso;
		}
		return reflectance;
	}
...
private:
	...
	Float m_pollution_content;
	Spectrum m_wavelengths;

	Spectrum m_refrections;
};

MTS_IMPLEMENT_CLASS_S(ART, false, BSDF)
MTS_EXPORT_PLUGIN(ART, "ART BRDF")
MTS_NAMESPACE_END