...
public:
	SunSkyEmitter(const Properties &props)
		...
		Float *data = (Float *)bitmap->getFloatData();
		...
		for (int y=0; y<bitmap->getHeight(); ++y) {
			...
			Float *target = data + y * bitmap->getWidth();
			...
			for (int x=0; x<bitmap->getWidth(); ++x) {
				...
				Spectrum skySpectrum = sky->evalEnvironment(ray);
				for (int i = 0; i < SPECTRUM_SAMPLES; i++){
					*target++ = skySpectrum[i];
				}
		...
		if (sunRadiusScale == 0) {
			...
			for (size_t i=0; i<nSamples; ++i) {
				...
				Spectrum tmpspec = value / std::max((Float) 1e-3f, sinTheta);
				for (int ii = 0; ii < SPECTRUM_SAMPLES; ii++){
					data[pos.x + pos.y * bitmap->getWidth() + ii] += tmpspec[ii];
				}
