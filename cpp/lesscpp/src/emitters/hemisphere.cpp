 ...
 */
class HemisphereEmitter : public Emitter {
public:
	HemisphereEmitter(const Properties &props) : Emitter(props) {
		m_type |= EOnSurface | EEnvironmentEmitter;
		m_radiance = props.getSpectrum("radiance", Spectrum::getD65());
		// cout<<m_radiance.toString()<<endl;
	}

	HemisphereEmitter(Stream *stream, InstanceManager *manager)
	...
	Spectrum samplePosition(PositionSamplingRecord &pRec,
		...
		Vector d = warp::squareToUniformHemisphereExt(sample);
	...
	Spectrum sampleRay(Ray &ray,
			...
			Float time) const {
		Vector v0 = warp::squareToUniformHemisphereExt(spatialSample);
		...
		ray.setTime(time);
	//	cout << ray.o.x << " " << ray.o.z << " " << ray.o.y <<" "<< ray.d.x << " " << ray.d.z << " " << ray.d.y<< endl;
		return m_radiance * (2 * M_PI * M_PI * m_geoBSphere.radius * m_geoBSphere.radius);
	}
	...
	Spectrum sampleDirect(DirectSamplingRecord &dRec,
		...
		if (!dRec.refN.isZero()) {
			...
			d = warp::squareToUniformHemisphereExt(sample);
	...
	Float pdfDirect(const DirectSamplingRecord &dRec) const {
		...
		if (!dRec.refN.isZero())
			...
			pdfSA = warp::squareToUniformHemispherePdf();
...
class HemisphereEmitterEmitterShader : public Shader {
public:
	HemisphereEmitterEmitterShader(Renderer *renderer, const Spectrum &radiance)
...
Shader *HemisphereEmitter::createShader(Renderer *renderer) const {
	return new HemisphereEmitterEmitterShader(renderer, m_radiance);
}

MTS_IMPLEMENT_CLASS(HemisphereEmitterEmitterShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(HemisphereEmitter, false, Emitter)
MTS_EXPORT_PLUGIN(HemisphereEmitter, "HemisphereEmitter emitter");
MTS_NAMESPACE_END
