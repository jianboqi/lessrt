...
class VegFluorMedium : public Medium {
public:
	//enum ELadType {
	//	ESpherical,  /// 球型分布
	//	EUniform,   /// 统一型
	...
	VegFluorMedium(const Properties& props)
		: Medium(props) {
	...
	VegFluorMedium(Stream* stream, InstanceManager* manager)
	...
	void configure() {
		...
		//std::string catch_out = this->getPhaseFunction()->getProperties().getString("Mb");
		if (IS_FLUSPECT_PRO) {
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mb"), "Mb", m_M.m_Mbi);
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mf"), "Mf", m_M.m_Mfi);
			parseSpectrumTxt(this->getPhaseFunction()->getProperties().getString("phi"), "phi", m_phiI);
			m_fqeI = this->getPhaseFunction()->getProperties().getFloat("fqe", 0.012);
		}
		else {
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mbi"), "Mbi", m_M.m_Mbi);
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mfi"), "Mfi", m_M.m_Mfi);
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mbii"), "Mbii", m_M.m_Mbii);
			parseEFMatrix(this->getPhaseFunction()->getProperties().getString("Mfii"), "Mfii", m_M.m_Mfii);
			parseSpectrumTxt(this->getPhaseFunction()->getProperties().getString("phiI"), "phiI", m_phiI);
			parseSpectrumTxt(this->getPhaseFunction()->getProperties().getString("phiII"), "phiII", m_phiII);
			m_fqeI = this->getPhaseFunction()->getProperties().getFloat("fqeI", 0.002);
			m_fqeII = this->getPhaseFunction()->getProperties().getFloat("fqeII", 0.01);
		}
		m_computedFluorSingleAlbedo.compute_AddForwardBackwardEF(m_M);
		m_computedSingleAlbedo = m_frontRef + m_transmittance;
		parseSpectrumTxt(this->getPhaseFunction()->getProperties().getString("kChlrel"), "kChlrel", m_kChlrel);
	}
	...
	Spectrum getVegetationSingleAlbedo(const Ray& ray) const {
		return  m_computedSingleAlbedo;// m_frontRef + m_transmittance;
	}

	FluorMatrix getFluorVegetationSingleAlbedo(const Ray& ray) const {
		return  m_computedFluorSingleAlbedo;// computePSISingleAlbedo();
	}
	...
	bool sampleDistanceWithTau(const Ray& ray,
		...
		Spectrum singleAlbedo = m_computedSingleAlbedo;//computeSingleAlbedo();
		FluorMatrix FluorsingleAlbedo = m_computedFluorSingleAlbedo;
		bool success = true;
		...
		if (sampledTau < tau) {
			...
			mRec.sigmaA = sigmaT * (Spectrum(1.0) - singleAlbedo);
			mRec.sigmaS = sigmaT * singleAlbedo;
			mRec.FluorsigmaS.compute_EqualMb_M1(FluorsingleAlbedo, sigmaT);
			mRec.time = ray.time;
...
private:
	...
	void parseEFMatrix(const std::string& file_name, const std::string& name, std::vector<Float>& m) {
		std::vector<std::string> s = tokenize(file_name, " ,;\n");
		if (s.size() == 0) {
	...
	void parseSpectrumTxt(const std::string& file_name, const std::string& name, Spectrum& m) {
		std::vector<std::string> s = tokenize(file_name, " ,;\n");
		if (s.size() == 0) {
			SLog(EError, "No %s were supplied!", name.c_str());
		}
...
private:
	...
	Spectrum m_computedSingleAlbedo;
	FluorMatrix m_computedFluorSingleAlbedo;
	FluorMatrixs m_M;
...
MTS_IMPLEMENT_CLASS_S(VegFluorMedium, false, Medium)
MTS_EXPORT_PLUGIN(VegFluorMedium, "Vegetation Fluor medium");
MTS_NAMESPACE_END
