...
void help() {
	...
	cout <<  "Usage: lessrt [options] <One or more scene XML files>" << endl;
	...
	cout <<  "               (e.g. when running LESS on a cluster. Default: 1)" << endl << endl;
...
int mitsuba_app(int argc, char **argv) {
	...
	try {
		...
		//log->addAppender(new StreamAppender(formatString("less.%s.log", nodeName.c_str())));
		if (!quietMode)
		...
		for (size_t i=0; i<hosts.size(); ++i) {
			...
			if (hostName.find("@") == std::string::npos) {
				...
				std::string path = "~/LESS"; // default path if not specified
				std::vector<std::string> tokens = tokenize(hostName, "@:");
