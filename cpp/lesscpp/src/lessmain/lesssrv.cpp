...
int lesssrv(int argc, char **argv) {
	int optchar;
	char *end_ptr = NULL;
	...
	try {
		...
		while ((optchar = getopt(argc, argv, "a:c:s:n:p:i:l:L:qhv")) != -1) {
			switch (optchar) {
				...
				case 'h':
				default:
					cout << "LESS Radiative Transfer program version " << "1.0"
						<< ", Copyright (c) " MTS_YEAR " Jianbo Qi" << endl;
					cout <<  "Usage: lesssrv [options]" << endl;
					...
					cout <<  "   -c hosts    Nesting: connect to additional lesssrv instances over a network." << endl;
					...
					cout <<  "                       'path' denotes the place where LESS is checked" << endl;
					cout <<  "                       out -- by default, \"~/LESS\" is used)" << endl << endl;
					cout <<  "   -s file     Connect to additional LESS servers specified in a file" << endl;
					...
					cout << " For documentation, please refer to http://ramm.bnu.edu.cn/projects/less/" << endl;
					return 0;
			}
		}
		...
		log->addAppender(new StreamAppender(formatString("lesssrv.%s.log", nodeName.c_str())));
		if (!quietMode)
		...
		for (size_t i=0; i<hosts.size(); ++i) {
			...
			if (hostName.find("@") == std::string::npos) {
				...
				cmdLine.push_back(formatString("bash -c 'cd %s; . setpath.sh; lesssrv -ls'", path.c_str()));
		...
		SLog(EInfo, "Enter lesssrv -h for more options");
