...
// sampling a hemisphere at top of the scene
// the original squareToUniformHemisphere samples a hemisphere in Z direction
Vector squareToUniformHemisphereExt(const Point2 &sample) {
	Float z = sample.x;
	Float tmp = math::safe_sqrt(1.0f - z*z);
	...
	return Vector(cosPhi * tmp, z, sinPhi * tmp);
}
