...
#define EXPORT_LIGHT_PROJECTION
...
#ifdef EXPORT_LIGHT_PROJECTION
	std::ofstream out("lightAccuracy.txt");
#endif
	for (int i = 0; i < shsampler->number_of_samples; i++)
	{
		Vector3f& direction = shsampler->shsamples[i].cartesian_coord;
...
#ifdef EXPORT_LIGHT_PROJECTION
		out << directionRadiance[1] << " " << estimatedRadiance_each_direction[1] << endl;
#endif
		rmse += diff*diff;
	}
#ifdef EXPORT_LIGHT_PROJECTION
...
void envaluateVisibilityProjectionAccuracy(StoreVisibility *visibilities, std::vector<Spectrum*> Tcoeffs, SHSampler* shsampler, int bands, const Scene* scene,int triindex)
{
	Spectrum rmse = Spectrum(0.0);
	std::ofstream out("visibility.txt");
	for (int i = 0; i < shsampler->number_of_samples; i++)
	{
		bool visible = visibilities[triindex].visibility[i];
		Spectrum directionRadiance;
		if (visible)
		{
			directionRadiance = Spectrum(1.0);
		}	
		else
		{
			directionRadiance = Spectrum(0.0);
		}

		Spectrum estimatedRadiance_each_direction = Spectrum(0.0); // for envaluate the performance of SH for approximate visibility
		for (int j = 0; j < bands*bands; j++)
		{
			float sh_function = shsampler->shsamples[i].sh_functions[j];
			estimatedRadiance_each_direction += Tcoeffs[triindex][j] * sh_function;
		}
		Spectrum diff = estimatedRadiance_each_direction - directionRadiance;
		out << directionRadiance[0] << " " << estimatedRadiance_each_direction[0] << endl;
		rmse += diff*diff;
	}
	out.close();
	rmse /= shsampler->number_of_samples;
	rmse = rmse.sqrt();
	cout << "visibility estimation RMSE: ";
	for (int i = 0; i < SPECTRUM_SAMPLES; i++)
	{
		cout << rmse[i] << " ";
	}
