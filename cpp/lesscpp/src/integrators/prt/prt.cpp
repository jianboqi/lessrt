...
public:
	...
	void do_precomputing(const Scene * scene)
	{
		...
		if (m_maxOrder > 1)
		{
			...
			for (int i = 1; i < m_maxOrder; i++)
			{
				if (i == 1)
				{
					HighOrderPrecomputing(scene, Tcoeffs, highorderTcoeffs[i - 1], BTcoeffs, BhighorderTcoeffs[i - 1]);
				}
				else
				{
					HighOrderPrecomputing(scene, highorderTcoeffs[i - 2], highorderTcoeffs[i - 1], BhighorderTcoeffs[i - 2], BhighorderTcoeffs[i - 1]);
				}
	...
	bool preprocess(const Scene *scene, RenderQueue *queue, const RenderJob *job,
		...
		cout << "m_totalTriangleCount:" << m_totalTriangleCount<< endl;
		...
		//back facet
		BTcoeffs.resize(m_totalTriangleCount);

		for (int i = 0; i < m_totalTriangleCount; i++)
		{
			BTcoeffs[i] = new Spectrum[m_shbands*m_shbands];
			for (int j = 0; j < m_shbands*m_shbands; j++)
				BTcoeffs[i][j] = Spectrum(0.0);
		}



		BhighorderTcoeffs.resize(m_maxOrder);
		//high order inilize
		for (int o = 0; o < m_maxOrder; o++)
		{
			BhighorderTcoeffs[o].resize(m_totalTriangleCount);
			for (int i = 0; i < m_totalTriangleCount; i++)
			{
				BhighorderTcoeffs[o][i] = new Spectrum[m_shbands*m_shbands];
				for (int j = 0; j < m_shbands*m_shbands; j++)
					BhighorderTcoeffs[o][i][j] = Spectrum(0.0);
			}
		}
		...
		//envaluateVisibilityProjectionAccuracy(visibilities,Tcoeffs, shsampler, m_shbands, scene, 500);
	...
	float dot(Vector n, Vector v) const
	{
	...
	//give a position and bsdf, envalute .this value has factor of 1/M_PI
	void envaluateReflectance_Trans(const BSDF *bsdf, Point center_point, Spectrum &frontR, Spectrum &backR, Spectrum &frontT, Spectrum &backT)
	{
		Intersection its_tmp;
		its_tmp.p = center_point;
		BSDFSamplingRecord bRecfr(its_tmp, Vector(0, 0, 1), Vector(0, 0, 1));
		frontR = bsdf->eval(bRecfr);

		BSDFSamplingRecord bRecbr(its_tmp, Vector(0, 0, -1), Vector(0, 0, -1));
		backR = bsdf->eval(bRecbr);

		BSDFSamplingRecord bRecft(its_tmp, Vector(0, 0, 1), Vector(0, 0, -1));
		frontT = bsdf->eval(bRecft);

		BSDFSamplingRecord bRecbt(its_tmp, Vector(0, 0, -1), Vector(0, 0, 1));
		backT = bsdf->eval(bRecbt);
	}

	// 
	Spectrum F(Vector outW,Vector incomeW, Normal surfaceNormal, Spectrum frontR, Spectrum backR, Spectrum frontT, Spectrum backT)
	{
		Spectrum Fws;
		float out_n = dot(surfaceNormal, outW);
		if (out_n >= 0)
		{
			//cout << (M_PI*frontR).toString() << endl;
			Fws = frontR*max(0, dot(surfaceNormal, incomeW)) + backT*max(0, -dot(surfaceNormal, incomeW));
		}
		else
		{
			//cout << "hello" << endl;
			Fws = frontT*max(0, dot(surfaceNormal, incomeW)) + backR*max(0, -dot(surfaceNormal, incomeW));
		}
		return Fws;
	}
	...
	void ProjectShadowed(const Scene *scene)
	{
		...
		for (int i = 0; i < trismeshs.size(); i++)
		{
			...
			Spectrum frontR, backR, frontT, backT;
#pragma omp parallel for
			for (int j = 0; j < triangleCount; j++)
			{
				...
				for (int s = 0; s < shsampler->number_of_samples; s++)
				{
					...
					if (!isIntersected)
					{
						if (shsample.cartesian_coord.y > 0)
						{
							//float cosine_term = max(0,dot(triNormal, shsample.cartesian_coord));
							envaluateReflectance_Trans(bsdf, center_point, frontR, backR, frontT, backT);
							for (int k = 0; k < m_shbands*m_shbands; k++)
							{
								float sh_function = shsample.sh_functions[k];
								Tcoeffs[triIndex][k] += sh_function*F(triNormal, shsample.cartesian_coord, triNormal, frontR, backR, frontT, backT);
								// for test
								//Tcoeffs[triIndex][k] += sh_function*Spectrum(1.0);
								BTcoeffs[triIndex][k] += sh_function*F(-triNormal, shsample.cartesian_coord, triNormal, frontR, backR, frontT, backT);
							}
						}
					}
					else
	...
	void HighOrderPrecomputing(const Scene* scene, std::vector<Spectrum*> &preOrder, std::vector<Spectrum*> &nextOrder,
		std::vector<Spectrum*> &BpreOrder, std::vector<Spectrum*> &BnextOrder)
	{
		std::vector<TriMesh*> trismeshs = scene->getMeshes();
		for (int i = 0; i < trismeshs.size(); i++)
		{
...
#pragma omp parallel for
			for (int j = 0; j < triangleCount; j++)
			{
				...
				for (int s = 0; s < shsampler->number_of_samples; s++)
				{
					...
					if (directionVisibility)
					{
						...
						for (int k = 0; k < m_shbands*m_shbands; k++)
						{
							nextOrder[triIndex][k] += preOrder[intersectedTriIdx][k] * F(triNormal, shsample.cartesian_coord, triNormal, frontR, backR, frontT, backT);
							BnextOrder[triIndex][k] += BpreOrder[intersectedTriIdx][k] * F(-triNormal, shsample.cartesian_coord, triNormal, frontR, backR, frontT, backT);
						}
					}
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
		...
		else
		{
			...
			Spectrum *TCoeff;
			float f_or_b = dot(its.wi, Vector(0, 0, 1));
			if (f_or_b>0)
			{
				TCoeff = Tcoeffs[tri.t_idx];
			}
			else
			{
				TCoeff = BTcoeffs[tri.t_idx];
			}
			
			for (int i = 0; i < m_shbands*m_shbands; i++)
			{
				Li += TCoeff[i] * lightCoeffs[i];
			}
			...
			if (m_maxOrder > 1)
			{
				for (int i = 0; i < m_maxOrder; i++)
				{
					...
					if (f_or_b > 0)
					{
						tranCoeff = highorderTcoeffs[i][tri.t_idx];
					}
					else
					{
						tranCoeff = BhighorderTcoeffs[i][tri.t_idx];
					}
					for (int j = 0; j < m_shbands*m_shbands; j++)
					{
...
private:
	...
	std::vector<Spectrum*> BTcoeffs; //back of a triangle
	std::vector<std::vector<Spectrum*> > BhighorderTcoeffs; // back of a triangle
