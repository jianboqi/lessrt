...
namespace PointCloudIncident {
	...
	struct DiscretePoint {
		...
		DiscretePoint() {}
		DiscretePoint(Float x, Float y, Float z, Float a, int i, int returnNum, int numberOfReturn, int bandIndex, Float incident, Float intercept) : x(x), y(y), z(z), a(a), i(i),
			returnNum(returnNum), numberOfReturn(numberOfReturn), bandIndex(bandIndex), incident(incident), intercept(intercept) {}
		...
		inline void load(Stream* stream) {
			...
			incident = stream->readFloat();
			intercept = stream->readFloat();
		}
		...
		inline void save(Stream* stream) const {
			...
			stream->writeFloat(incident);
			stream->writeFloat(intercept);
		}
	};
	...
	struct IncidentRecord {
		...
		IncidentRecord() {}
		IncidentRecord(Float t, Spectrum w) : t(t), w(w) {}

		bool operator<(const IncidentRecord& b) const {
			return t < b.t;
		}
	};
	...
	public:
		...
		void prepare() {
			...
			m_incidentRecords.reserve(m_circleBeamSampler->getSize());
		...
		void process(const WorkUnit *workUnit, WorkResult *workResult, const bool &stop) {
			...
			for (size_t index = wu->getRangeStart(); index <= wu->getRangeEnd() && !stop; ++index) {
				...
				if (getWaveformResult(pulseOrigin, pulseDirection, wr, indexOfWR)) { // for each pulse waveform
					sortAndSumIncidentRecords();
		...
		void sortAndSumIncidentRecords() {
			sort(m_incidentRecords.begin(), m_incidentRecords.end());
			for (int i = m_incidentRecords.size() - 2; i >= 0; --i) {
				m_incidentRecords[i].w += m_incidentRecords[i + 1].w;
			}
		}

		// not overrided methods

		bool getWaveformResult(const Point& pulseOrigin, const Vector& pulseDirection, PointCloudWorkResult *wr,size_t indexOfWR) {
			...
			while (m_circleBeamSampler->hasNext()) {
				...
				hit |= m_scene->rayIntersect(ray, its);
				m_incidentRecords.emplace_back(IncidentRecord(its.t, w));
		...
		void getPoints(const Point& pulseOrigin, const Vector& pulseDirection, PointCloudWorkResult *wr, 
			...
			for (int k = 0; k < SPECTRUM_SAMPLES; k++) {
				...
				//size_t n = (size_t)(round(2 * 3 * par[2] / (1e-9 * 1e9) - 1) / 2);

				//Float energy = 0;
				//for (int j = 0; j <= 2*n+1; j++) {
				//	int index = int(j + par[1]-n);
				//	energy += waveform[index];
				//}
				...
				for (int i = 0; 3 * i < par.size(); i++) {
					...
					p = o + d * t;
					Float offset = 4*par[i * 3 + 2] * step;
					if (i > 0) {
						offset = (t - prev_t) * 0.5;
					}
					prev_t = t;
					Float incident = getIncidentWeight(t, k, offset);
					wr->m_points[indexOfWR].push_back(new DiscretePoint(p.x, p.y, p.z, a, indexOfPulse, i + 1, numReturns, k+1, incident, 0.f));
				}

				// intercept
				int i = wr->m_points[indexOfWR].size() - par.size() / 3;
				for (; i < wr->m_points[indexOfWR].size() - 1; ++i) {
					wr->m_points[indexOfWR][i]->intercept = wr->m_points[indexOfWR][i]->incident - wr->m_points[indexOfWR][i + 1]->incident;
				}
				wr->m_points[indexOfWR][i]->intercept = wr->m_points[indexOfWR][i]->incident - getIncidentWeight(std::numeric_limits<Float>::infinity(), k, step);
			}
		...
		Float getIncidentWeight(Float t, size_t k, Float offset) {
			auto r = lower_bound(m_incidentRecords.begin(), m_incidentRecords.end(), IncidentRecord(t - offset, Spectrum(0.f)));
			if (r == m_incidentRecords.end()) {
				return 0.f;
			}
			return r->w[k];
		}
		...
		void naivePointDetectionForTLS(const Point& pulseOrigin, const Vector& pulseDirection, PointCloudWorkResult* wr,
			...
			if (maxEnergy > 0) {
				...
				p = o + d * t;
				Float incident = getIncidentWeight(t, 0, step);
				wr->m_points[indexOfWR].push_back(new DiscretePoint(p.x, p.y, p.z, a, indexOfPulse,1,1,1, incident, incident - getIncidentWeight(std::numeric_limits<Float>::infinity(), 0, step)));
			}
	...
	public:
		...
		std::vector<IncidentRecord> m_incidentRecords;
	...
	public:
		...
		void setupProgressReporter(const std::string& progressText, size_t numPulses, const void* progressReporterPayload) {
			...
			m_minIncident = numeric_limits<int>::max();
			m_minIntercept = numeric_limits<int>::max();
		}
			...
			for (int i = 0; i < range->getSize(); i++) {
				...
				for (int j = 0; j < points.size(); j++) {
					...
					DiscretePoint p(point->x, point->y, point->z, point->a, point->i, point->returnNum, point->numberOfReturn,
						point->bandIndex, point->incident, point->intercept);
					if(point->a > 0)
						if (point->a < m_minIntensity) m_minIntensity = point->a;
					if(point->incident > 0)
						if (point->incident < m_minIncident) m_minIncident = point->incident;
					if(point->intercept > 0)
						if (point->intercept < m_minIntercept) m_minIntercept = point->intercept;
					m_pointCloud.push(p);
				}
			}
		}
		...
		void outputPointCloudToOneFile(std::string outName = "cloud.txt") {
			...
			Float incident_scale = 1;
			int inci_scale_times = 0;
			if (m_minIncident < 0.01) {
				while (true) {
					incident_scale *= 10;
					m_minIncident *= 10;
					inci_scale_times += 1;
					if (m_minIncident >= 1.0) {
						break;
					}
				}
			}

			Float intercept_scale = 1;
			int inter_scale_times = 0;
			if (m_minIntercept < 0.01) {
				while (true) {
					intercept_scale *= 10;
					m_minIntercept *= 10;
					inter_scale_times += 1;
					if (m_minIntercept >= 1.0) {
						break;
					}
				}
			}
			...
			DiscretePoint p;
			fout << "         X          Y          Z PulseIndex BandIndex ReturnNum NumReturns     Intensity(e-" << scale_times <<")"<<"       Incident(e-" << inci_scale_times << ")" << "      Intercept(e-" << inter_scale_times << ")"<< endl;
			while (m_pointCloud.try_pop(p)) {
				fout << std::fixed << std::setprecision(4) << std::setw(10) << 0.5 * m_sceneXSzie - p.x << " " <<
					...
					std::setprecision(5) << std::scientific << std::setw(20) << p.a * intensity_scale <<
					std::setprecision(5) << std::scientific << std::setw(20) << p.incident * incident_scale <<
					std::setprecision(5) << std::scientific << std::setw(20) << p.intercept * intercept_scale <<
					endl;
			}
	...
	public:
		...
		Float m_minIntensity, m_minIncident, m_minIntercept;
