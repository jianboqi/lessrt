...
public:
	...
	void renderBlock(const Scene *scene, const Sensor *sensor,
		...
		uint8_t *snapshot = (uint8_t *)alloca(sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2)*(2 * borderSize + 1)*(2 * borderSize + 1));
		...
		for (size_t i=0; i<points.size(); ++i) {
			...
			for (int y=0; y<2*borderSize+1; ++y) {
				uint8_t *src = (target + sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2)*((y + points[i].y)* block->getBitmap()->getWidth() + points[i].x));
				uint8_t *dst = snapshot + sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2)*y*(2 * borderSize + 1);
				memcpy(dst, src, sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2) * (2 * borderSize + 1));
			}
			...
			for (int y=0; y<2*borderSize+1; ++y) {
				uint8_t *dst = target + sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2)*((y + points[i].y)* block->getBitmap()->getWidth() + points[i].x);
				uint8_t *backup = snapshot + sizeof(uint8_t)*(SPECTRUM_SAMPLES + 2)*y*(2 * borderSize + 1);
