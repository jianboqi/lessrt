...
MTS_NAMESPACE_BEGIN
//PMRT photon mapping radiative transfer
 ...
 * }
 */
class PMRTIntegrator : public SamplingIntegrator {
public:
	PMRTIntegrator(const Properties &props) : SamplingIntegrator(props),
	...
	PMRTIntegrator(Stream *stream, InstanceManager *manager)
	...
	bool preprocess(const Scene *scene, RenderQueue *queue, const RenderJob *job,
		...
		if (m_globalPhotonMap.get() == NULL && m_globalPhotons > 0) {
			...
			/*ref<FileStream> fstream = new FileStream("d:\\dump_pm1",
				FileStream::EReadOnly);
			ref<InstanceManager> manager = new InstanceManager();
			m_globalPhotonMap = static_cast<PhotonMap*> (manager->getInstance(fstream));*/

			/*ref<FileStream> fstream = new FileStream("d:\\dump_pm1",
				FileStream::ETruncWrite);
			ref<InstanceManager> manager = new InstanceManager();
			manager->serialize(fstream, m_globalPhotonMap.get());*/
			m_globalPhotonMap->dumpText("d:\\pos_dir.txt");
		}
	...
	Spectrum Li_new(const RayDifferential &ray, RadianceQueryRecord &rRec) const {
...
MTS_IMPLEMENT_CLASS_S(PMRTIntegrator, false, SamplingIntegrator)
MTS_EXPORT_PLUGIN(PMRTIntegrator, "PMRT integrator");
MTS_NAMESPACE_END
