...
float LightProbeAccess(Vector3f coord, const Scene* scene)
{
	RayDifferential ray;
	ray.setOrigin(Point(0, 0, 0));
	ray.setDirection(coord);
	float value = scene->evalEnvironment(ray)[0];
	return value;
}

void projectLightintoSH(float* coeffs, SHSampler* shsampler, int bands, const Scene* scene) //only for one band
{
	...
	for (int i = 0; i < shsampler->number_of_samples; i++)
	{
		...
		float directionRadiance = LightProbeAccess(direction, scene);
