 ...
 * }
 */
class LESSParticleTracer : public Integrator {
public:
	LESSParticleTracer(const Properties &props) : Integrator(props) {
	...
	LESSParticleTracer(Stream *stream, InstanceManager *manager)
	...
	bool render(Scene *scene, RenderQueue *queue,
		...
		Log(EInfo, "Starting simulation job (%ix%i, " SIZE_T_FMT " samples, " SIZE_T_FMT
		...
		ref<ParallelProcess> process = new LESSParticleProcess(
...
MTS_IMPLEMENT_CLASS_S(LESSParticleTracer, false, Integrator)
MTS_EXPORT_PLUGIN(LESSParticleTracer, "LESS particle tracer");
MTS_NAMESPACE_END
