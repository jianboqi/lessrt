...
#if !defined(__LESSPTRACER_PROC_H)
#define __LESSPTRACER_PROC_H
 ...
 */
class LESSParticleWorkResult : public ImageBlock {
public:
	inline LESSParticleWorkResult(const Vector2i &res, const ReconstructionFilter *filter)
	 : ImageBlock(Bitmap::ESpectrum, res, filter) {
		setOffset(Point2i(0, 0));
		setSize(res);
		m_range = new RangeWorkUnit();
		m_directionRadiance = new Spectrum[36 * 15];
		m_directionPhotonCount.resize(36 * 15);
	}
...
protected:
	/// Virtual destructor
	virtual ~LESSParticleWorkResult() { }
protected:
	ref<RangeWorkUnit> m_range;
public:
	Spectrum * m_directionRadiance;
	std::vector<int>  m_directionPhotonCount;
};
 ...
 */
class LESSParticleWorker : public ParticleTracer {
public:
	inline LESSParticleWorker(int maxDepth, int maxPathDepth,
		int rrDepth, bool bruteForce) : ParticleTracer(maxDepth, rrDepth, true),
		m_maxPathDepth(maxPathDepth), m_bruteForce(bruteForce) { }

	LESSParticleWorker(Stream *stream, InstanceManager *manager);
...
protected:
	/// Virtual destructor
	virtual ~LESSParticleWorker() { }
private:
	ref<const Sensor> m_sensor;
	ref<const ReconstructionFilter> m_rfilter;
	ref<LESSParticleWorkResult> m_workResult;
	int m_maxPathDepth;
	bool m_bruteForce;
};
 ...
 * a group of machines
 */
class LESSParticleProcess : public ParticleProcess {
public:
	LESSParticleProcess(const RenderJob *job, RenderQueue *queue,
...
//directional radiance
	Spectrum * m_directionalRadiance;
	std::vector<int> m_directionalPhotonCount;
	int m_maxDepth;
	int m_maxPathDepth;
	int m_rrDepth;
	bool m_bruteForce;
