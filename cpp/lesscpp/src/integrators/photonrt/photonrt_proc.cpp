...
void CapturePhotonProcess::bindResource(const std::string& name, int id) {
	...
	if (name == "sensor") {
		if (m_hasUpDownProducts) {
			...
			m_accum_downwell = new ImageBlock(Bitmap::ESpectrum, m_film_downwell->getCropSize(), NULL); //���з���
			m_accum_downwell->clear();

			m_accum_upwell = new ImageBlock(Bitmap::ESpectrum, m_film_downwell->getCropSize(), NULL); //���з���
