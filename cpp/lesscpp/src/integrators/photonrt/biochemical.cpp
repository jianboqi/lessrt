...
class BiochemicalTracer : public Integrator {
public:
	BiochemicalTracer(const Properties& props) : Integrator(props) {
		...
		m_hasBiochemicalProduct = props.getInteger("BiochemicalProduct", 1);
	...
	BiochemicalTracer(Stream* stream, InstanceManager* manager)
	...
	bool render(Scene* scene, RenderQueue* queue,
		...
		ref<ParallelProcess> process = new CapturePhotonProcess(
			...
			m_numberOfDirections, m_hasfPARProducts, m_hasfSunlitLeafProducts, m_boolOutParEachBand,
...
MTS_IMPLEMENT_CLASS_S(BiochemicalTracer, false, Integrator)
MTS_EXPORT_PLUGIN(BiochemicalTracer, "Photon RT Bio Tracer");
MTS_NAMESPACE_END
