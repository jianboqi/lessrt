...
*/
template <typename Value, typename InnerValue, size_t logblockSize = 2> class BlockedArray {
public:
	...
	*/
	void alloc(const Vector2i &size) {
		...
		//	* blockSize * blockSize * sizeof(Value::Scalar)*Value::dim);
		m_data = new Value[m_xBlocks * m_yBlocks * blockSize * blockSize];
	...
	*/
	template <typename AltValue> void init(const AltValue *data) {
		InnerValue *innerdata = (InnerValue*) data;
		for (int y = 0; y < m_size.y; ++y)
		{
			for (int x = 0; x < m_size.x; ++x)
			{
				Value &value = (*this)(x, y);
				for (int i = 0; i<AltValue::dim; ++i) {
					value[i] = (typename Value::Scalar)(*innerdata);
					innerdata++;
				}
			}
	...
	template <typename AltValue> void init(const AltValue *data,
		...
		for (int y = 0; y<m_size.y; ++y) {
			for (int x = 0; x<m_size.x; ++x) {
				Value &value = (*this)(x, y);
				for (int i = 0; i<AltValue::dim; ++i) {
					min[i] = std::min(min[i], (*innerdata));
					max[i] = std::max(max[i], (*innerdata));
					avg[i] += (*innerdata);
					value[i] = (typename Value::Scalar)(*innerdata);
					innerdata++;
				}
	...
	*/
	template <typename AltValue> void copyTo(AltValue *data) const {
		...
		{
			for (int x = 0; x < m_size.x; ++x){
				const Value &value = (*this)(x, y);
				for (int i = 0; i<AltValue::dim; ++i) {
					*innerdata = (typename AltValue::Scalar)(value[i]); 
					//*innerdata = reinterpret_cast <AltValue::Scalar>(value[i]);
					innerdata++;
				}
			}
	...
	*/
	void cleanup() {
		...
		//	memset(&(this->operator()(0, y + m_size.y)), 0, /*Value::dim*sizeof(Value::Scalar) **/sizeof(Value)* m_xBlocks * blockSize);

		//if (unusedColsRight > 0) {
		//	for (int y = 0; y < (int)(m_yBlocks * blockSize); ++y)
		//		for (int x = 0; x < (int)unusedColsRight; ++x)
		//			memset(&(this->operator()(x + m_size.x, y)), 0, sizeof(Value)/*Value::dim*sizeof(Value::Scalar)*/);
		//}
	}
	...
	inline static size_t bufferSize(const Vector2i &size) {
		...
		return fullWidth * fullHeight * sizeof(Value)/*Value::dim*sizeof(Value::Scalar)*/;
	}

	/// Return the size of the allocated buffer
	inline size_t getBufferSize() const {
		return m_xBlocks * m_yBlocks * blockSize * blockSize *sizeof(Value) /*Value::dim*sizeof(Value::Scalar)*/;
	}
