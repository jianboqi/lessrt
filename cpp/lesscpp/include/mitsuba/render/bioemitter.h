...
#pragma once
#if !defined(__MITSUBA_RENDER_BIOEMITTER_H_)
#define __MITSUBA_RENDER_BIOEMITTER_H_
...
/**
 * \brief Abstract radiance/importance bioemitter interface
 *
 * This class implements an abstract interface to all bioemitters in Mitsuba. The
 * term bioemitter is interpreted in a loose bidirectional sense here, where
 * luminaires and sensors are both considered to be bioemitters of radiance and
 ...
 * (a generalization of direct illumination sampling to both bioemitters \a and
 * sensors). A direct sampling is given an arbitrary input position in the
 * scene and in turn returns a sampled bioemitter position and direction, which
 ...
 * this reason, every bioemitter instance keeps a reference to a medium
 * (or \c NULL when it is surrounded by vacuum).
 *
 * \ingroup librender
 */
class MTS_EXPORT_RENDER AbstractBioemitter : public ConfigurableObject {
public:
	/**
	 * \brief Flags used to classify the emission profile of
	 * different types of bioemitters
	 */
	enum EBioemitterType {
	 ...
	 * and maps it to a position on the surface of the bioemitter.
	 *
	 * Some implementations may choose to implement extra functionality
	 * based on the value of \c extra: for instance, Sensors
	 * (which are a subclass of \ref AbstractBiobioemitter) perform uniform
	 ...
	 * scene, sample an bioemitter position that contributes towards it.
	 *
	 * Given an arbitrary reference point in the scene, this method
	 * samples a position on the bioemitter that has a nonzero contribution
	 * towards that point.
	 ...
	 * and the position on the bioemitter.
	 *
	 * The default implementation throws an exception.
	 *
	 * \param dRec
	 ...
	 *    any geometric terms between the bioemitter and the reference point.
	 */
	...
	/**
	 * \brief Does the bioemitter lie on some kind of surface?
	 */
	...
	/// Return a pointer to the medium that surrounds the bioemitter
	inline Medium *getMedium() { return m_medium; }

	/// Return a pointer to the medium that surrounds the bioemitter (const version)
	inline const Medium *getMedium() const { return m_medium.get(); }

	/// Return the shape, to which the bioemitter is currently attached
	inline Shape *getShape() { return m_shape; }

	/// Return the shape, to which the bioemitter is currently attached (const version)
	...
	/**
	 * \brief Create a special shape that represents the bioemitter
	 *
	 * Some types of bioemitters are inherently associated with a surface, yet
	 ...
	 * represents bioemitters in a generalized bidirectional sense, which
	 * includes sensors).
	 *
	 ...
	 * extents of the bioemitter
	 */
	virtual AABB getAABB() const = 0;

	/// Set the medium that surrounds the bioemitter
	inline void setMedium(Medium *medium) { m_medium = medium; }

	/// Serialize this bioemitter to a binary data stream
	...
	MTS_DECLARE_CLASS()
protected:
	/// Construct a new bioemitter instance
	AbstractBioemitter(const Properties &props);

	/// Unserialize a bioemitter instance from a binary data stream
	AbstractBioemitter(Stream *stream, InstanceManager *manager);

	/// Virtual destructor
	virtual ~AbstractBioemitter();
protected:
...
struct bio_param
{
	...
	Float Vcmax = 60;
	Float BallBerrySlope = 8;
	Float BallBerry0 = 0.01;
	Float Rd = 0.015;
	Float Kn0 = 2.48;
	Float Knalpha = 2.83;
	Float Knbeta = 0.114;
	Float po0 = 0.7595054480275167;
	Float RH = 0.26637910310002433;
	Float Cs = 0.00037345;
	Float Gamma_star = 7.791848289726934e-05;
	Float MM_consts = 0.0021260405752797575;
	Float Vs_C3 = 37.32817020379738;
	Float Je_Q = 0.37975272401375837;
	Float Kpepcase = 1689888.54128516;
};

/**
 * \brief Abstract radiance bioemitter interface
 *
 * This class provides an abstract interface to all bioemitter plugins in Mitsuba.
 ...
 * \ingroup librender
 */
class MTS_EXPORT_RENDER Bioemitter : public AbstractBioemitter, public HWResource {
public:
	/**
	 ...
	 *
	 * \sa AbstractBioemitter::EBioemitterType
	 */
	enum EBioemitterFlags {
		/// Is this an environment bioemitter, such as a HDRI map?
		EFvCBBioemitter = 0x010,
		/// Is this an planck bioemitter with thermal radiation
		EMD12Bioemitter = 0x020
	};

	// =================================================================
	//! @{ \name Additional bioemitter-related sampling and query functions
	// =================================================================
	...
	virtual Bioemitter& operator=(const Bioemitter& other);

	virtual void set_m_Q(Float Q);

	virtual int get_triangleCount() const;
	virtual Float* get_trianglesArea() const;
	virtual void set_triangleCount(int triangleCount);
	virtual void set_trianglesArea(Float* trianglesArea);

	virtual Spectrum eval(const Intersection &its, const Vector &d) const;
	virtual bio_param get_value_sun() const;
	virtual std::string get_shape_name()const;
	virtual std::string get_palnt_type()const;
	virtual Spectrum get_kChlrel()const;
	virtual void compute_photosynthesis(bool hasFluor, Float AtsPressure, Float AtsCO2, Float AtsO2, Float Q, unsigned int ShadePhotonNum, unsigned int SunPhotonNum, Float& A, Float& eta);
 	/**
	...
	/**
	 * \brief Return a bitmap representation of the bioemitter
	 *
	...
	// =============================================================
	//! @{ \name Functionality related to environment bioemitters
	// =============================================================

	/// Is this an environment emitter? (e.g. an HDRI environment map?)
	inline std::string getBioemitterType() const {
		if (m_type & EFvCBBioemitter) {
			return "FvCB";
		}
		else if (m_type & EMD12Bioemitter) {
			return "MD12";
		}
	}
	/**
	 * \brief Return the radiant emittance from an environment bioemitter
	 *
	 ...
	 * the scene has been determined to contain an environment bioemitter.
	 *
	 * The default implementation throws an exception.
	 *
	 * \param ray
	 ...
	 * illumination sampling density of an environment bioemitter.
	 *
	 ...
	 * environment bioemitter needs to be known by a multiple importance
	 * sampling technique.
	 *
	 * The default implementation throws an exception.
	 *
	...
	/// Is this a compound bioemitter consisting of several sub-objects?
	virtual bool isCompound() const;

	/**
	 * \brief Return a sub-element of a compound bioemitter.
	 *
	 * When expanding bioemitters, the scene will repeatedly call this
	 * function with increasing indices. Returning \a NULL indicates
	 * that no more are available.
	 */
	virtual Bioemitter *getElement(size_t index);
...
protected:
	/// Construct a new bioemitter instance
	Bioemitter(const Properties &props);

	/// Unserialize a bioemitter instance from a binary data stream
	Bioemitter(Stream *stream, InstanceManager *manager);
...
#endif /* __MITSUBA_RENDER_BIOEMITTER_H_ */
